...
def train(args,model,optimizer,with_eval):
    ...
    for epoch in epoch_iterator:
        shuffle(feature_file_list) # train目录下的文件名列表
        file_iterator = trange(int(len(feature_file_list)), desc="Train File")
        for file_idx in file_iterator:
            file_name = feature_file_list[file_idx] # 获取到每个文件名
            file_path_name = os.path.join(train_feature_dir,file_name) # 获取文件名的绝对路径
            assert os.path.isfile(file_path_name)
            train_dataset = get_features_from_cache(file_path_name)
